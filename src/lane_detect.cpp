...
LaneDetector::LaneDetector(rclcpp::Node::SharedPtr nh)
	:
    	/******* recording log *******/
	...
	this->declare_parameter("Calibration/matrix/a",matrix[0], 3.2918100682757097e+02);
	this->declare_parameter("Calibration/matrix/b",matrix[1], 0.);
	this->declare_parameter("Calibration/matrix/c",matrix[2], 320.);
	this->declare_parameter("Calibration/matrix/d",matrix[3], 0.);
	this->declare_parameter("Calibration/matrix/e",matrix[4], 3.2918100682757097e+02);
	this->declare_parameter("Calibration/matrix/f",matrix[5], 240.);
	this->declare_parameter("Calibration/matrix/g",matrix[6], 0.);
	this->declare_parameter("Calibration/matrix/h",matrix[7], 0.);
	this->declare_parameter("Calibration/matrix/i",matrix[8], 1.);

	this->declare_parameter("Calibration/dist_coef/a",dist_coef[0], -3.2566540239089398e-01);
	this->declare_parameter("Calibration/dist_coef/b",dist_coef[1], 1.1504807178349362e-01);
	this->declare_parameter("Calibration/dist_coef/c",dist_coef[2], 0.);
	this->declare_parameter("Calibration/dist_coef/d",dist_coef[3], 0.);
	this->declare_parameter("Calibration/dist_coef/e",dist_coef[4], -2.1908791800876997e-02);
	...
	this->declare_parameter("ROI/width", width_, 640);
	this->declare_parameter("ROI/height", height_, 480);
	...
	this->declare_parameter("ROI/top_gap",t_gap, 0.336f);
	this->declare_parameter("ROI/bot_gap",b_gap, 0.078f);
	this->declare_parameter("ROI/top_height",t_height, 0.903f);
	this->declare_parameter("ROI/bot_height",b_height, 0.528f);
	this->declare_parameter("ROI/extra_f",f_extra, 0.0f);
	this->declare_parameter("ROI/extra_b",b_extra, 0.0f);
	this->declare_parameter("ROI/extra_up",extra_up, 0);
	this->declare_parameter("ROI/extra_down",extra_down, 0);
	this->declare_parameter("ROI/dynamic_roi",option_, true);
	this->declare_parameter("ROI/threshold",threshold_, 128);
	distance_ = 0;
	...
	this->declare_parameter("params/K", K_, 0.15f);
	this->declare_parameter("params/a/a", a_[0], 0.);
	this->declare_parameter("params/a/b", a_[1], -0.37169);
	this->declare_parameter("params/a/c", a_[2], 1.2602);
	this->declare_parameter("params/a/d", a_[3], -1.5161);
	this->declare_parameter("params/a/e", a_[4], 0.70696);
	this->declare_parameter("params/b/a", b_[0], 0.);
	this->declare_parameter("params/b/b", b_[1], -1.7536);
	this->declare_parameter("params/b/c", b_[2], 5.0931);
	this->declare_parameter("params/b/d", b_[3], -4.9047);
	this->declare_parameter("params/b/e", b_[4], 1.6722);
	
	LoadParams();
}
       
} // Class LaneDetector end

        /* View Option */
bool ScaleTruckController::readParameters() {
	this->declare_parameter("image_view_opencv", viewImage_, true);
	this->declare_parameter("image_view/wait_key_delay", waitKeyDelay_, 3);
	this->declare_parameter("image_view/enable_console_output", enableConsoleOutput_, true);
}
...
void LaneDetector::LoadParams(void) {
	this->declare_parameter("LaneDetector/eL_height",eL_height_, 1.0f);	
	this->declare_parameter("LaneDetector/e1_height",e1_height_, 1.0f);	
	this->declare_parameter("LaneDetector/trust_height",trust_height_, 1.0f);	
	this->declare_parameter("LaneDetector/lp",lp_, 756.0f);	
	this->declare_parameter("LaneDetector/steer_angle",SteerAngle_, 0.0f);
}
...
int main(int argc, char* argv[]){
    ...
    std::shared_ptr<rclcpp::Node> nostd::make_shared<LaneDetect::LaneDetector>();
    rclcpp::spin(node);
    rclcpp::shutdown();
    return 0;
}
